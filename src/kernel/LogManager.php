...
class LogManager implements LoggerInterface
{
    ...
    protected function createEmergencyLogger()
    {
        ...
        try {
            $hadler = new StreamHandler(
                sys_get_temp_dir() . '/baiduai/baiduai.log', $this->level(['level' => 'debug'])
            );

        } catch (Exception $e) {
            $hadler =  new NullHandler();
        }

        return new Monolog('baiduai', $this->prepareHandlers([$hadler]));
    }
    ...
    {
        try {
            $hadler = new StreamHandler($config['path'], $this->level($config));
        } catch (Exception $e) {
            $hadler =  new NullHandler();
        }

        return new Monolog($this->parseChannel($config), [$this->prepareHandler($hadler)]);
    }
    ...
    protected function createSyslogDriver(array $config)
    {
        return new Monolog($this->parseChannel($config), [
            $this->prepareHandler(new SyslogHandler(
                    'BaiduAi', $config['facility'] ?? LOG_USER, $this->level($config))
            ),
        ]);
    }
